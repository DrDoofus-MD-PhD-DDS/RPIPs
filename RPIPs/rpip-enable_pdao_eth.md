...
RPIP-72 introduced a `pdao_share`, which allows the protocol to direct part of ETH staking rewards of megapools to the pDAO. This proposal enables the pDAO to spend this ETH by voting on one-time spends and recurring payments in ETH. It also generalizes these mechanisms to payments for any ERC20.

## Specification

This RPIP extends functionality of two proposal types of the on-chain pDAO voting system introduced in RPIP-33.
### One-Time Spend

This proposal type deals with one-time spends of ETH or ERC20 tokens from the protocol’s treasury.
...
4. **Token Address (optional):** In case of a token spend, the token address.
...
This proposal type deals with the creation and modification of recurring ETH or ERC2-20 token payment contracts.
...
7. **Token Address (optional):** In case of a token spend, the token address.
8. **Amount Per Interval:** The amount of ETH to send per reward interval.
...
## Rationale
This proposal aims to enable spending of ETH with as little complexity as possible, by mirroring the existing implementation of spending RPL from the treasury. Generalizing to ERC-20 doesn't appear to add complexity and is included to future-proof.
## Copyright
