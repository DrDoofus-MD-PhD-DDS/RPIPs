---
rpip: TBD
title: pDAO Signalling Governance
description: Establish a platform independent signalling process, adopt RocketDash, and provide an emergency method for changing signalling platforms.
...
The long term intention is for the on-chain pDAO system to support additional forms of governance. Until that work is complete, RocketDash provides a community developed alternative for conducting signalling votes.

Existing governance documents identify Snapshot directly rather than defining a replaceable signalling platform. This creates a continuity problem if Snapshot, RocketDash, or another future platform becomes unavailable or untrustworthy.
...
### Recognized Signalling Platform

* The **Recognized Signalling Platform** SHALL be the platform used for pDAO signalling votes under [RPIP-4](RPIP-4.md).
* RocketDash at `https://rocketdash.net/vote` SHALL be the initial Recognized Signalling Platform under this RPIP.
* References to Snapshot voting in existing RPIPs SHALL refer to the Recognized Signalling Platform unless the reference is specifically historical or technical to Snapshot.
* The current and historical Recognized Signalling Platforms SHALL be recorded in this RPIP.
...
* Only the Lead Vote Administrator and current Authorized Vote Publishers MAY publish official votes on the Recognized Signalling Platform.
...
* [RPIP-33](RPIP-33.md) SHALL recognize an **Emergency Signalling Platform Change** as an allowed on-chain proposal type.
...
* An Emergency Ballot System proposal MAY only:
  ...
  * change the Recognized Signalling Platform; and
  * appoint the vote administrators needed to operate the replacement platform.
* The proposal message MUST plainly identify:

  * the Recognized Signalling Platform to be replaced;
  * the proposed Recognized Signalling Platform;
...
* Emergency Signalling Platform Change proposals:
  * SHALL require a sentiment poll, but
  * SHALL NOT require a separate signalling vote.
...
* Tooling (preferably Smart node) SHALL provide a method to create an Emergency Signalling Platform Change proposal with the required message.
* The proposal SHALL use the existing RPIP-33 voting process and passing criteria.
* When the proposal becomes successful under the RPIP-33 voting process, the platform to be replaced MUST match the Recognized Signalling Platform then in effect. If it matches, the proposed platform and vote administration changes SHALL take effect; otherwise, the proposal SHALL have no governance effect.
* If multiple Emergency Signalling Platform Change proposals become successful in the same block, they SHALL be applied in ascending proposal ID order.
...
Substantive changes to the signalling process, platform change process, Emergency Ballot System, or vote administration authority MUST be approved by pDAO vote.
...
RPIP-4 directly identifies Snapshot as the platform used for governance votes. This RPIP instead defines the Recognized Signalling Platform as a replaceable component of the governance process.
...
RocketDash is intended as an interim system while broader on-chain governance functionality is developed. However, this RPIP does not require another migration on a predetermined schedule. RocketDash may remain the Recognized Signalling Platform for as long as the pDAO considers it suitable.

### Normal platform changes

Under ordinary circumstances, the current Recognized Signalling Platform should be used to approve its replacement.

For example, RocketDash could conduct a vote to return to Snapshot, move to another signalling platform, or transfer signalling votes to a future version of the on-chain pDAO system.
...
A signalling system cannot be relied upon to authorize its own replacement after it has become unavailable or untrustworthy. The Emergency Ballot System provides an independent fallback using the existing on-chain pDAO. The EBS is only to be used for signalling platform changes and only when the community has agreed via discussion and sentiment poll that the existing system is no longer trustworthy or useable. 
...
The Emergency Ballot System is limited to signalling platform continuity. It cannot pass an ordinary RPIP, change a protocol parameter, spend treasury funds, or modify the Security Council.
...
Existing RPIP-4 voting rules remain in effect. References in existing RPIPs to Snapshot as the official venue for pDAO signalling votes SHALL instead refer to the Recognized Signalling Platform. Historical references, literal Snapshot URLs, and technical details specific to Snapshot remain unchanged and SHALL NOT be interpreted as requirements for RocketDash or a future Recognized Signalling Platform.

## Security Considerations

### Signalling platform integrity

The Recognized Signalling Platform must correctly apply pDAO voting power, delegation, eligibility, quorum, and outcome rules.
...
Authorized Vote Publishers can advance or withhold official signalling votes. Appointments should therefore be limited to trusted parties and recorded promptly.
